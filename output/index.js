...
function RGBToHSL(r, g, b) {
    ...
    g /= 255;
    b /= 255;
    let cmin = Math.min(r, g, b), cmax = Math.max(r, g, b), c = cmax - cmin, h = 0, s = 0, l = 0;
    if (c == 0)
        h = 0;
    else if (cmax == r)
        h = ((g - b) / c) % 6;
...
function coatedtest() {
    ...
    for (var i = 0; i < pms_coated.length; i++) {
        if (rgb == pms_coated[i].rgb) {
            console.log(pms_coated[i].code);
        }
    }
}
function saturation(rgb, s) {
    ...
    mid = [0, 1, 2].filter(function (i) { return i !== min && i !== max; })[0], a = rgb[max] - rgb[min], b = rgb[mid] - rgb[min], x = rgb[max], arr = [x, x, x];
    if (min === max) {
